...
import { formStorageService } from '../utils/storageService.js';
...
const formConfigs = {
  ...
  'YOUTUBE SCRIPT': {
    ...
    'DIGITAL PRODUCT VIDEO': {
      ...
      tones: toneOptions,
      toneQuestionIndex: 3,
      numberQuestionIndex: 2  // Only question 3 (index 2) should be a number field
    },
    ...
    'YOUTUBE TITLES': {
      ...
      tones: toneOptions,
      toneQuestionIndex: 2,
      numberQuestionIndex: 1  // Only question 2 (index 1) should be a number field
    },
    'PHYSICAL PRODUCT VIDEO': {
      questions: [
        'What is the physical product you want to promote in the video?',
...
function PromptForm({ formTypeFilter }) {
  ...
  const isNumberField = (question, fieldIndex) => {
    ...
    // Restrict numeric detection to explicit quantitative terms only.
    // Do NOT infer numbers from generic nouns like "features", "benefits", or "ideas".
    ...
    const hasNumberKeyword = numberKeywords.some(keyword => {
      // Create a regex pattern that matches the keyword as a whole word or phrase
    ...
    // For advertisement forms, ensure "features/benefits/ideas" style questions are treated as text
    // even if phrased with quantities like "top 3".
    const isAdsNonNumeric = /features|benefits|ideas/.test(questionLower);

    return (hasNumberKeyword && !isAdsNonNumeric) || isExplicitNumber;
  };
  ...
  const generatePrompt = () => {
    ...
    analyticsService.trackFormUsage(formType);

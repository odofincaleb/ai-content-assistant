...
const PromptFormScreen: React.FC = () => {
  ...
  const [dynamicFields, setDynamicFields] = useState<{ [key: number]: string }>({});
  ...
  const handleChange = (index: number, value: string) => {
    ...
    // Handle dynamic fields logic
    if (formConfig.dynamicFields && index === formConfig.dynamicFields.triggerFieldIndex) {
      const sceneCount = parseInt(value) || 0;
      const newDynamicFields: { [key: number]: string } = {};
      ...
      // Update answers array to accommodate dynamic fields
      const updatedAnswers = [...newAnswers];
      // Remove any existing dynamic fields beyond the base questions
      updatedAnswers.splice(formConfig.dynamicFields.targetFieldIndex + 1);
      // Add empty values for dynamic fields
      for (let j = 0; j < sceneCount; j++) {
        updatedAnswers[formConfig.dynamicFields.targetFieldIndex + 1 + j] = '';
      }
  ...
  const generatePrompt = async () => {
    ...
    try {
      ...
      if (formConfig.dynamicFields && Object.keys(dynamicFields).length > 0) {
        const sceneDescriptions = Object.keys(dynamicFields).map((dynamicIndex) => {
          const fieldIndex = formConfig.dynamicFields.targetFieldIndex + 1 + parseInt(dynamicIndex);
          return answers[fieldIndex] || '';
        }).filter(desc => desc.trim() !== '').join('\n\n');
        
        // Replace the dynamic field placeholder with all scene descriptions
        prompt = prompt.replace(new RegExp(`\\{${formConfig.dynamicFields.targetFieldIndex + 1}\\}`, 'g'), sceneDescriptions);
      }
      
      // Replace regular field placeholders
      answers.forEach((answer, index) => {
        prompt = prompt.replace(new RegExp(`\\{${index + 1}\\}`, 'g'), answer);
      });
    ...
    >
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.formSection}>
          ...
          {formConfig.dynamicFields && Object.keys(dynamicFields).length > 0 && 
            Object.keys(dynamicFields).map((dynamicIndex) => {
              const fieldIndex = formConfig.dynamicFields.targetFieldIndex + 1 + parseInt(dynamicIndex);
              const sceneNumber = parseInt(dynamicIndex) + 1;
              return (
                <View key={`dynamic-${dynamicIndex}`} style={styles.fieldContainer}>
                  <Text style={styles.questionLabel}>
                    {formConfig.dynamicFields.targetFieldIndex + 1}. {formConfig.dynamicFields.fieldPrefix} {sceneNumber} {formConfig.dynamicFields.fieldSuffix}
                  </Text>
                  <TextInput
                    style={[
                      styles.input,
                      errors[fieldIndex] && styles.inputError
                    ]}
                    placeholder={`Example: [${(sceneNumber-1)*8}–${sceneNumber*8}s] Scene ${sceneNumber} description`}
                    placeholderTextColor="#999"
                    value={answers[fieldIndex] || ''}
                    onChangeText={(value) => handleChange(fieldIndex, value)}
                    onBlur={() => handleBlur(fieldIndex)}
                    multiline={true}
                    numberOfLines={3}
                  />
                  {errors[fieldIndex] && (
                    <Text style={styles.errorText}>{errors[fieldIndex]}</Text>
                  )}
                </View>
              );
            })
